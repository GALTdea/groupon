...
!(function () {
  ...
  var success = function (xhr, options) {
    ...
    /////////////////////// HIJACK /////////////////////////////
    $(document).find(".btn.minus").on("click", function(event) {
      var amount_field = $(this).parent().parent().find('#amount');
      if (Number(amount_field.val()) > 0) {
        amount_field.val(Number(amount_field.val()) - 1);
      }
    });
    $(document).find(".btn.plus").on("click", function(event) {
      console.log($(this));
      var amount_field = $(this).parent().parent().find('#amount');
      amount_field.val(Number(amount_field.val()) + 1);
    });
    $(document).find(".control-item").on("click", function(event) {
      $(this).parent().find('.control-item input').prop('disabled', true);

      var input = $(this).parent().find('.active input');
